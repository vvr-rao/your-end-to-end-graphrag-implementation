...
def entity_extract(
    ...
    *,
    ...
    system = (
        ...
        "shares its subject matter. Whenever you answer \"NONE_OF_THESE\" you "
        "MUST also fill in \"proposed_type\" with a short type name (e.g. "
        "\"Canal\", \"Person\"). \"proposed_type\" is REQUIRED for every "
        "\"NONE_OF_THESE\" answer and must never be empty or omitted.\n"
...
def relationship_extract(
    ...
    system = (
        ...
        "plainly states is just as much an error as inventing one, and it is "
        "the more common failure: a passage naming eight entities usually "
        "asserts more than two or three links between them.\n"
        "  So before you return, walk the ENTITIES list and ask of each one: "
        "does the passage say anything about how this entity stands to "
        "another entity on the list -- who employs or leads it, who makes or "
        "owns it, what it treats or causes or competes with, where it sits? "
        "If a quotable sentence names both and states the link, and a "
        "predicate fits, emit it. You are not being asked to guess; you are "
        "being asked not to stop early.\n"
        "  A person named alongside their employer, a product named alongside "
        "its maker, and a drug named alongside what it treats are the three "
...
_FACTS_FIRST_RULE = (
    ...
    "  - COPY each citation character-for-character from inside the "
    "square brackets of the EVIDENCE block. Never construct, complete, "
    "guess or adjust an identifier. Specifically:\n"
    "      * The ids look sequential -- they are NOT. Neighbouring "
    "numbers are different documents or do not exist.\n"
    "      * An id is an OPAQUE code such as "
    "'viao:Chunk_5270af92eec6958d_ft_0000'. The readable document name "
    "in the '(document: ...)' tag is NOT part of it and must never "
    "appear inside a citation. Do not swap the code for the name, or "
    "build an id out of the name -- 'viao:Chunk_005_Some_Paper_Title_"
    "ft_0002' is not a real identifier and resolves to nothing.\n"
    "      * If a fact is worth stating but you cannot find its exact "
    "id in the block, state it WITHOUT a citation rather than inventing "
    "one.\n"
...
_DEEP_RESEARCH_SECTIONS = (
    ...
    "CLAIMS\n"
    "  List the significant claims surfaced for the question, SPLIT into "
    "the two labelled groups below by whether the source backed the "
    "claim. Use the `evidence_status` and `claim_source` metadata on "
    "Claim artifacts when present. Line format for both groups: "
    "\"<Claim>. [Stated by <source> in <doc>]\".\n"
    "  WITH EVIDENCE:\n"
    "    Claims the source supported with reasoning, data, or a citation "
    "(evidence_status = backed or partial). If none, write \"None.\"\n"
    "  WITHOUT EVIDENCE:\n"
    "    Claims stated with no supporting reasoning or backing "
    "(evidence_status = unbacked). If none, write \"None.\"\n\n"
